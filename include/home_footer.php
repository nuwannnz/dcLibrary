 <div class="footer site-accent-bg-dark" style="padding-top:5px;">
                <div class="content center-align-bl" style="min-height:20px;" > 
                    <ul class="menu" style="position:absolute;top:5px;"> 
                        <li><a class="link-button" href="<?php echo $header_paths['public'] . '/contact_us.php'; ?>">Contact us</a></li>
                        <li><a class="link-button" href="<?php echo $header_paths['public'] . '/help.php'; ?>">Help</a></li>
                        <li><a class="link-button" href="<?php echo $header_paths['public'] . '/admin_login.php'; ?>">Staff login</a></li>
                    </ul>
                    <span class="site-accent-fg" style="position:absolute;top:5px;right:10px;" >&copy dcLibrary 2016</span>     
                </div>
            </div> <!-- end of footer-->
        </div> <!-- end of root container-->
    </body>
</html>